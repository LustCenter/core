...
alias ark="$HOME/core-lustchain/packages/core/bin/run"
echo 'alias lust="$HOME/core-lustchain/packages/core/bin/run"' >> ~/.bashrc

rm -rf "$HOME/core-lustchain"
git clone "https://github.com/LustCenter/core.git" "$HOME/core-lustchain" || FAILED="Y"
if [ "$FAILED" == "Y" ]; then
    echo "Failed to fetch core repo with origin 'https://github.com/LustCenter/core.git'"

    exit 1
fi

cd "$HOME/core-lustchain"
...
rm -rf "$HOME/.config/@lust"
rm -rf "$HOME/.config/@lust"
rm -rf "$HOME/.config/lust-core"
